classdef CustomAxsym < quadind.geometry.AxsymGeometry
    ...
    %   radius = @(theta) 1 + 0.15*cos(2*theta);
    %   a_func = @(theta) radius(theta).*sin(theta);
    %   c_func = @(theta) radius(theta).*cos(theta);
    %   da_func = @(theta) -0.3*sin(2*theta).*sin(theta) + ...
    %       radius(theta).*cos(theta);
    %   dc_func = @(theta) -0.3*sin(2*theta).*cos(theta) - ...
    %       radius(theta).*sin(theta);
    %   
    %   geom = quadind.geometry.CustomAxsym(a_func, c_func, da_func, dc_func);
    %
    ...
    methods
        function obj = CustomAxsym(aFunc, cFunc, daFunc, dcFunc, varargin)
            ...
            %   'name' - Optional name for display
            %
            % The functions must accept vector inputs. IndicatorEvaluator also
            % requires equatorial symmetry and validates it at construction.
