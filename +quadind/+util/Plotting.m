classdef Plotting
    ...
    %
    % Provides plotting functions for error-indicator evaluation validation:
    %   - Quadrature region maps
    %   - Error vs distance scatter plots
    %   - Error contours with indicator overlays
    %   - Diverging colormaps
    %
    % Usage:
    %   quadind.util.Plotting.plotQuadRegions(...)
    %   quadind.util.Plotting.plotErrorContour(...)
    %
    ...
    methods (Static)
        function plotQuadRegions(xv, zv, classification, mask_ext, geom, varargin)
            ...
            %   classification - Struct from IndicatorEvaluator.evaluate with .masks, .requiresSpecialQuadrature
            ...
            sqMask(mask_ext) = classification.requiresSpecialQuadrature;
            ...
            quadind.util.Plotting.drawGeometryOutline(geom);
            ...
            % Apply colormap
            colormap(gca, quadind.util.Plotting.divergingColormap(nfac + 1));
        ...
        function plotErrorVsDistance(u, uref, targets, gridSurf, classification, tol, varargin)
            ...
            %   classification - Struct with .masks, .requiresSpecialQuadrature
            ...
            [itheta,iphi] = quadind.indicator.UniformIndicatorBuilder.findNearestNodes(gridSurf, targets);
            ...
            if any(classification.requiresSpecialQuadrature)
                plot(dist(classification.requiresSpecialQuadrature), err(classification.requiresSpecialQuadrature), '.', 'MarkerSize', 8);
            end
        ...
        function plotErrorContour(xv, zv, error, indicators, mask_ext, geom, varargin)
            % PLOTERRORCONTOUR Filled contour of error with indicator overlay
            %
            %   Single upsampling factor:
            %   plotErrorContour(xv, zv, error, indicators, mask_ext, geom)
            %   plotErrorContour(..., 'uniformIndicators', estuni)
            %   plotErrorContour(..., 'levels', [0:-2:-10])
            %
            %   Multiple upsampling factors (one figure per factor):
            %   plotErrorContour(xv, zv, errors_cell, indicators_cell, mask_ext, geom, ...
            ...
            %   indicators  - [M×1] error indicators at exterior points, OR
            %                cell array {k} of [M×1] vectors (one per upsampling factor)
            ...
            %                        required when error/indicators are cell arrays
            %   'uniformIndicators' - [M×1] uniform (unmodified) indicators for overlay
            %                        (single factor only)
            %   'levels'           - Contour levels (default: [0:-2:-10])
            %   'FontSize'         - Font size (default: 16)
            
            p = inputParser;
            addParameter(p, 'uniformIndicators', []);
            ...
            if iscell(error)
                ...
                for k = 1:nfac
                    ...
                    % Per-factor indicators (cell or shared single array)
                    ...
                    quadind.util.Plotting.plotSingleFactor_(xv, zv, error{k}, est_k, ...
            ...
            quadind.util.Plotting.plotSingleFactor_(xv, zv, error, indicators, ...
                mask_ext, geom, levels, FS, showLabels);
        end
        ...
        function plotSurfaceMesh(gridOrGeom, varargin)
            ...
            % Resolve grid
            if isa(gridOrGeom, 'quadind.grid.AxsymGrid')
                g = gridOrGeom;
            else
                % Geometry supplied — build a temporary grid
                g = quadind.grid.AxsymGrid(gridOrGeom, ...
        ...
        function map = divergingColormap(m)
            ...
            map = zeros(m, 3);
            for i = 1:m
                map(i, :) = quadind.util.Plotting.divergingMapValue(s(i), rgb1, rgb2);
            end
        end
    ...
    methods (Static, Access = private)
        function plotSingleFactor_(xv, zv, error, indicators, mask_ext, geom, levels, FS, showLabels)
            ...
            % contours of the error indicator in the current axes.
            %
            % Inputs:
            %   xv, zv    - Coordinate vectors
            %   error     - [M×1] error values at exterior points
            %   indicators - [M×1] error indicators at exterior points
            %   mask_ext  - Logical mask for exterior points
            %   geom      - AxsymGeometry object
            ...
            % Line contour of indicators (black solid)
            ...
            % Draw surface mesh
            quadind.util.Plotting.plotSurfaceMesh(geom,'FlipYZ', true);
        ...
        function rgb = divergingMapValue(s, rgb1, rgb2)
            ...
            lab1 = quadind.util.Plotting.rgbToLab(rgb1);
            lab2 = quadind.util.Plotting.rgbToLab(rgb2);
            
            msh1 = quadind.util.Plotting.labToMsh(lab1);
            msh2 = quadind.util.Plotting.labToMsh(lab2);
            
            % Insert white midpoint for saturated distinct colors
            if msh1(2) > 0.05 && msh2(2) > 0.05 && ...
               quadind.util.Plotting.angleDiff(msh1(3), msh2(3)) > 0.33*pi
            ...
            if msh1(2) < 0.05 && msh2(2) > 0.05
                msh1(3) = quadind.util.Plotting.adjustHue(msh2, msh1(1));
            elseif msh2(2) < 0.05 && msh1(2) > 0.05
                msh2(3) = quadind.util.Plotting.adjustHue(msh1, msh2(1));
            end
            ...
            % Convert back
            labTmp = quadind.util.Plotting.mshToLab(mshTmp);
            rgb = quadind.util.Plotting.labToRgb(labTmp);
        end
        ...
        function lab = rgbToLab(rgb)
            xyz = quadind.util.Plotting.rgbToXyz(rgb);
            lab = quadind.util.Plotting.xyzToLab(xyz);
        end
        
        function rgb = labToRgb(lab)
            xyz = quadind.util.Plotting.labToXyz(lab);
            rgb = quadind.util.Plotting.xyzToRgb(xyz);
        end
