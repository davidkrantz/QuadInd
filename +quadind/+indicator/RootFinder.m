classdef RootFinder
    % ROOTFINDER Complex root finding for error-indicator evaluation
    %
    % Provides static methods for computing complex roots in phi and theta
    % directions, which are needed for error-indicator evaluation.
    %
    ...
    methods (Static)
        function [phi0, Gphi] = computePhiRoot(geometry, targets, theta_star)
            ...
            %   Gphi - [M×1] geometric factors for error-indicator evaluation
        ...
        function [theta0, Gtheta] = computeThetaRoot(geometry, targets, phi_star)
            % COMPUTETHETAROOT Compute complex theta roots
            %
            %   [theta0, Gtheta] = computeThetaRoot(geometry, targets, phi_star)
            %
