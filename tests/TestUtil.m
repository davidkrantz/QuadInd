classdef TestUtil < QuadIndTestCase
%TESTUTIL Unit tests for +quadind/+util module
%
%   Tests for Config, GaussLegendre, GaussLaguerre8, and Diagnostics
    ...
    methods (Test)
        function test_config_defaults(obj)
            ...
            cfg = quadind.util.Config();  % Create Config object
            
            obj.assertTrue(isa(cfg, 'quadind.util.Config'), 'Config should return Config object');
            obj.assertEqual(cfg.nth, 40);
            obj.assertEqual(cfg.nph, 60);
            obj.assertAlmostEqual(cfg.tol, 1e-6);
            obj.assertTrue(isnan(cfg.interpolatedRootResidualTolerance));
        end
        
        function test_config_get(obj)
            %TEST_CONFIG_GET Test Config with overrides
            cfg = quadind.util.Config('nth', 100);
            obj.assertEqual(cfg.nth, 100);
            
            % Default when not specified
            cfg2 = quadind.util.Config();
            obj.assertEqual(cfg2.nth, 40);
            obj.assertError(@() quadind.util.Config('notAProperty', 1), ...
                'quadind:Config:unknownProperty');
            obj.assertError(@() quadind.util.Config('nth'), ...
                'quadind:Config:invalidArguments');
        end
        
        function test_gauss_legendre_nodes(obj)
            %TEST_GAUSS_LEGENDRE_NODES Verify GL nodes are in [-1, 1]
            [x, w] = quadind.util.GaussLegendre(10, -1, 1);
        ...
        function test_gauss_legendre_symmetry(obj)
            ...
            [x, w] = quadind.util.GaussLegendre(8, -1, 1);
        ...
        function test_gauss_laguerre_hardcoded(obj)
            ...
            [x, w] = quadind.util.GaussLaguerre8();
        ...
        function test_diagnostics_checkDensity(obj)
            ...
            q = ones(40*60, 3);
            obj.assertNoError(@() quadind.util.Diagnostics.checkDensityResolution(q, 60));
        ...
        function test_geometryResolution_spheroid_resolved(obj)
            ...
            result = quadind.util.Diagnostics.checkGeometryResolution(geom, 40);
        ...
        function test_geometryResolution_capsule_resolved(obj)
            ...
            geom = quadind.geometry.Capsule('R', 1, 'L', 6, 'kappa', 4);
            result = quadind.util.Diagnostics.checkGeometryResolution(geom, 60);
        ...
        function test_geometryResolution_underresolved(obj)
            ...
            geom = quadind.geometry.Capsule('R', 1, 'L', 6, 'kappa', 20);
            result = quadind.util.Diagnostics.checkGeometryResolution(geom, 4);
        ...
        function test_geometryResolution_result_fields(obj)
            ...
            result = quadind.util.Diagnostics.checkGeometryResolution(geom, 10);
