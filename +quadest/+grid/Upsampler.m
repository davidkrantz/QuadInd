classdef Upsampler
    ...
    %
    % Usage:
    %   coarseGrid = quadest.grid.AxsymGrid(geom, 'nth', 40, 'nph', 60);
    %   fineGrid = coarseGrid.upsample(2);  % 80×120 grid
    %   interp = quadest.grid.Upsampler(coarseGrid, fineGrid);
    %   
    %   density_fine = interp.apply(density_coarse);
    %
    % See also: quadest.grid.AxsymGrid
    ...
    methods
        function obj = Upsampler(sourceGrid, targetGrid)
            ...
            arguments
                sourceGrid (1,1) quadest.grid.AxsymGrid
                targetGrid (1,1) quadest.grid.AxsymGrid
            end
