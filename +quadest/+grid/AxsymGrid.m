classdef AxsymGrid < handle
    ...
    %   grid = quadest.grid.AxsymGrid(geom, 'nth', 40, 'nph', 60);
    %
    %   % Access grid data
    ...
    %
    % See also: quadest.geometry.AxsymGeometry, quadest.grid.Upsampler
    ...
    methods
        function obj = AxsymGrid(geometry, varargin)
            ...
            addParameter(p, 'nth', cfg.nth, @(x) isscalar(x) && x > 0 && mod(x,1) == 0);
            addParameter(p, 'nph', cfg.nph, @(x) isscalar(x) && x > 0 && mod(x,1) == 0);
            parse(p, varargin{:});
        ...
        function newGrid = upsample(obj, factor)
            ...
            newGrid = quadest.grid.AxsymGrid(obj.geometry, ...
                'nth', factor * obj.nth, ...
                'nph', factor * obj.nph);
        end
        ...
        function interpolator = getInterpolator(obj, targetGrid)
            ...
            %
            % See also: quadest.grid.Upsampler
            
            interpolator = quadest.grid.Upsampler(obj, targetGrid);
        end
    ...
    methods (Access = private)
        function buildGrid(obj)
            ...
            [obj.theta, obj.wtheta] = quadest.util.GaussLegendre(obj.nth, 0, pi);
