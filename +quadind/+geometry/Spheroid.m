classdef Spheroid < quadind.geometry.AxsymGeometry
    ...
    %
    % Usage:
    %   geom = quadind.geometry.Spheroid('a', 0.05, 'c', 0.1);  % prolate
    %   geom = quadind.geometry.Spheroid();  % defaults: a=0.05, c=0.1
    %
    % The spheroid has analytic formulas for:
    %   - Interior/exterior test
    %   - Complex theta root (quartic solution)
    %
    % See also: quadind.geometry.AxsymGeometry, quadind.geometry.Peanut
    ...
    methods
        ...
        function theta0 = findThetaRoot(obj, targets, phi)
            ...
            if abs(delta) <= sqrt(eps) * max(obj.a^2, obj.c^2)
                ...
                theta0 = findThetaRoot@quadind.geometry.AxsymGeometry( ...
                    obj, targets, phi);
                return;
            end
            ...
            %         theta0(ii) = findThetaRoot@quadind.geometry.AxsymGeometry( ...
